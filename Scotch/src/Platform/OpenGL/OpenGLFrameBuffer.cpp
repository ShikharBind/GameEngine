...
namespace Scotch {
	...
	void OpenGLFrameBuffer::Resize(uint32_t width, uint32_t height)
	{
		...
		{
			SH_CORE_WARN("Attepted to resize framebuffer to ({0}, {1})", width, height);
			return;
		}
	...
	void OpenGLFrameBuffer::Invalidate()
	{
		...
		{
			SH_CORE_ASSERT(m_ColorAttachments.size() <= 4, "More than 4 color attachments on Framebuffer");
		...
		SH_CORE_ASSERT(glCheckFramebufferStatus(GL_FRAMEBUFFER) == GL_FRAMEBUFFER_COMPLETE, "Framebuffer is incomplete!");
